...
Key optimizations for sm_100/121a:
...
def _fused_add_rmsnorm_h4096(
    ...
    # x and residual are loaded as float32 for RMSNorm stability
    x = tl.load(x_ptr + row * H + cols, mask=mask, other=0.0, num_stages=num_stages).to(tl.float32)
    res = tl.load(residual_ptr + row * H + cols, mask=mask, other=0.0, num_stages=num_stages).to(tl.float32)

    # Compute h = x + residual
    h = x + res

    # Compute variance: var = mean(h^2)
    ...
    # Load weights
    w = tl.load(weight_ptr + cols, mask=mask, other=0.0, num_stages=num_stages).to(tl.float32)

    # Compute output: y = h * rstd * w
    y = h * rstd * w
...
def run(x, residual, weight):
    ...
    # Use a power-of-2 block size that fits within the 101376 byte SMEM limit
    # For H=4096, BLOCK_H=4096 fits easily (4096 * 4 * 3 = 49KB)
    BLOCK_H = triton.next_power_of_2(H)
    _fused_add_rmsnorm_h4096[(M,)](
        x, residual, weight, y, H, 1e-6,
        BLOCK_H=BLOCK_H, num_stages=3, num_warps=8
    )
    return y